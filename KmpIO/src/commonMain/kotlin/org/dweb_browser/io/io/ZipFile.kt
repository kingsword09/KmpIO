...
class ZipFile(
  ...
  override suspend fun addEntryBuffered(
    entry: ZipEntry,
    inputBlock: suspend () -> ByteBuffer
  ) {
    addEntry(entry) {
  ...
  private suspend fun writeEntry(
    entry: ZipEntry,
    content: suspend (entry: ZipEntry) -> WriteResult
  ) {
  ...
  override suspend fun extractToDirectory(
    ...
    useEntries { zipEntry ->
      if (filter?.invoke(zipEntry) != false) {
        val name = mapPath?.invoke(zipEntry) ?: zipEntry.name
        val f = File(
          when (directory.fullPath.endsWith("/")) {
            true -> directory.fullPath
            else -> directory.fullPath + "/"
          }, name
        )
  ...
  private suspend fun decompress(
    ...
    entry.directories.apply {
      ...
      if (crc.result != entry.directory.crc32) {
        throw ZipException(
          "CRC32 values don't match. Entry CRC: ${entry.directory.crc32.toString(16)}, content CRC: ${
            crc.result.toString(
              16
            )
          }"
  ...
  private suspend fun zipOneDirectory(
    directory: File,
    shallow: Boolean,
    parentPath: String,
    filter: ((pathName: String) -> Boolean)?
